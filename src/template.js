(function () {
  ...
  function protectMathDelimiters(text) {
    if (typeof text !== "string") return "";
    return text
      .replaceAll("\\(", "@@ANKI_MD_MATH_INLINE_OPEN@@")
      .replaceAll("\\)", "@@ANKI_MD_MATH_INLINE_CLOSE@@")
      .replaceAll("\\[", "@@ANKI_MD_MATH_DISPLAY_OPEN@@")
      .replaceAll("\\]", "@@ANKI_MD_MATH_DISPLAY_CLOSE@@");
  }
  ...
  function renderMarkdown(element) {
    ...
    try {
      ...
      const text = protectMathDelimiters(protectedDisplayMath.text);
      debug(`🔄 开始渲染 Markdown 内容，长度：${text.length}`);

      const rendered = restoreDisplayMathBlocks(
        restoreMathDelimiters(md.render(text)),
