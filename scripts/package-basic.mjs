...
const releaseConfig = JSON.parse(
  await readFile(path.join(root, "config/basic-release.json"), "utf8"),
);
...
const stagingDirectory = await mkdtemp(
  path.join(os.tmpdir(), "anki-markdown-basic-"),
);
...
try {
  ...
  await Promise.all([mkdir(mediaDirectory), mkdir(releaseDirectory, {recursive: true})]);
  ...
  await run(uv, [
    "run",
    "--script",
    "scripts/build-basic-apkg.py",
    "--config",
    "config/basic-release.json",
    "--front",
    "dist/basic/front.html",
    "--back",
    "dist/basic/back.html",
    "--styling",
    "dist/basic/styling.css",
    "--media-directory",
    mediaDirectory,
    "--output",
    artifactPath,
