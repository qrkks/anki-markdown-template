...
- 根据 `Back` 中的全部标题生成响应式浮动大纲。标题层级上方固定提供“正面”和“背面”入口；无标题的背面开场正文激活“背面”，到达标题后再高亮对应章节。大纲会在桌面侧栏和无障碍移动端抽屉之间自动切换；`Front` 继续保持自由格式且无需标题。
- 宽度不超过 600px 时默认使用 23px 正文，可通过 `--mb-mobile-font-size` 独立调整；桌面及标签字号保持原样。
